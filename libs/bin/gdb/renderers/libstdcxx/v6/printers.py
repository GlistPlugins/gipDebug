...
# Flag indicating that types should be omitted in pretty printers' to_string() results
_cidr_omit_type_inside_to_string = False

def cidr_run_with_omitting_types_inside_to_string(fn):
    """
    Runs the provided function with _cidr_omit_type_inside_to_string set to True
    """
    global _cidr_omit_type_inside_to_string
    orig = _cidr_omit_type_inside_to_string
    try:
        _cidr_omit_type_inside_to_string = True
        return fn()
    finally:
        _cidr_omit_type_inside_to_string = orig
...
class SharedPointerPrinter(printer_base):
    ...
    def to_string(self):
        ...
        if _cidr_omit_type_inside_to_string: return '{%s}' % state
...
class UniquePointerPrinter(printer_base):
    ...
    def to_string(self):
        if _cidr_omit_type_inside_to_string: return ''
...
class StdListPrinter(printer_base):
    ...
    def to_string(self):
        ...
        if headnode['_M_next'] == headnode.address:
            if _cidr_omit_type_inside_to_string: return '{empty}'
            return 'empty %s' % (self._typename)
        if _cidr_omit_type_inside_to_string:
            if any(f.name == '_M_size' for f in headnode.type.fields()):
                return '{%s}' % num_elements(int(headnode['_M_size']))
            return ''
...
class NodeIteratorPrinter(printer_base):
    ...
    def to_string(self):
        if not self._val['_M_node']:
            if _cidr_omit_type_inside_to_string: return '{non-dereferenceable}'
...
class StdVectorPrinter(printer_base):
    ...
    def to_string(self):
        ...
        if self._is_bool:
            ...
            if _cidr_omit_type_inside_to_string: return ('{length %d, capacity %d}' % (int(length), int(capacity)))
            return ('%s<bool> of length %d, capacity %d'
                    % (self._typename, int(length), int(capacity)))
        else:
            if _cidr_omit_type_inside_to_string: return ('{length %d, capacity %d}' % (int(finish - start), int(end - start)))
...
class StdBitReferencePrinter(printer_base):
    ...
    def to_string(self):
        if not self._val['_M_p']:
            if _cidr_omit_type_inside_to_string: return '{invalid}'
...
class StdStackOrQueuePrinter(printer_base):
    ...
    def to_string(self):
        if _cidr_omit_type_inside_to_string: return self._visualizer.to_string()
...
class StdMapPrinter(printer_base):
    ...
    def to_string(self):
        if _cidr_omit_type_inside_to_string: return '{%s}' % num_elements(len(RbtreeIterator(self._val)))
...
class StdDequePrinter(printer_base):
    ...
    def to_string(self):
        ...
        if _cidr_omit_type_inside_to_string: return '{%s}' % num_elements(long(size))
...
class StdStringStreamPrinter(printer_base):
    ...
    def to_string(self):
        if self._broken: return None
        if self._was_redirected:
            if _cidr_omit_type_inside_to_string: return '{redirected to %s}' % self._streambuf.dereference()
...
class Tr1UnorderedSetPrinter(printer_base):
    ...
    def to_string(self):
        ...
        if _cidr_omit_type_inside_to_string: return '{%s}' % num_elements(count)
...
class StdExpAnyPrinter(SingleObjContainerPrinter):
    ...
    def to_string(self):
        if self._contained_type is None:
            if _cidr_omit_type_inside_to_string: return '{no contained value}'
    ...
    def to_string(self):
        ...
        if hasattr(self._visualizer, 'children'):
            ...
            return "%s containing %s" % (self._typename,
                                         self._visualizer.to_string())
        if _cidr_omit_type_inside_to_string:
            if hasattr(self._visualizer, 'to_string'):
                return self._visualizer.to_string()
            return ''
...
class StdVariantPrinter(SingleObjContainerPrinter):
    ...
    def to_string(self):
        if self._contained_value is None:
            ...
            return "%s [no contained value]" % self._typename
        if hasattr(self._visualizer, 'children'):
            if _cidr_omit_type_inside_to_string: return '{index %d, value %s}' % (self._index, self._visualizer.to_string())
            return "%s [index %d] containing %s" % (self._typename, self._index,
                                                    self._visualizer.to_string())
        if _cidr_omit_type_inside_to_string: return '{index %d}' % self._index
...
class StdExpPathPrinter(printer_base):
    ...
    def to_string(self):
        ...
        if self._num_cmpts == 0:
            ...
            if t:
                path = '%s [%s]' % (path, t)
        if _cidr_omit_type_inside_to_string: return '%s' % path
...
class StdPathPrinter(printer_base):
    ...
    def _path_type(self):
        if self._type is None: return None
...
class StdErrorCodePrinter(printer_base):
    ...
    def to_string(self):
        ...
        if name is not None:
            ...
            name = alt_name
        if _cidr_omit_type_inside_to_string: return '{%s: %s}' % (name, strval)
...
class StdSpanPrinter(printer_base):
    ...
    def to_string(self):
        if _cidr_omit_type_inside_to_string: return '{length %d}' % self._size
...
class StdAtomicPrinter(printer_base):
    ...
    def to_string(self):
        ...
        if self._value_type.code == gdb.TYPE_CODE_INT:
            ...
            val = self._val['_M_i']
        if _cidr_omit_type_inside_to_string: return '%s' % val
...
class StdFormatArgsPrinter(printer_base):
    ...
    def to_string(self):
        ...
        if size == 1:
            if _cidr_omit_type_inside_to_string: return '{1 argument}'
            return "%s with 1 argument" % (typ)
        if size == 0:
            size = self._val['_M_unpacked_size']
        if _cidr_omit_type_inside_to_string: return '{%d arguments}' % size
...
class StdLocalePrinter(printer_base):
    ...
    def to_string(self):
        ...
        if _cidr_omit_type_inside_to_string: return '{"%s"%s}' % (name, mod)
